...
class HotelsDAO(BaseDAO):
    ...
    async def find_all(cls, location: str, date_from: date, date_to: date):
        ...
        booked_rooms = (
            select(Bookings.room_id, func.count(Bookings.room_id).label("number_of_rooms_booked"))
        ...
        booked_hotels = (
            select(Rooms.hotel_id, func.sum(
                Rooms.quantity - func.coalesce(booked_rooms.c.number_of_rooms_booked, 0)
            ).label('number_of_rooms_left'))
            .select_from(Rooms)
        ...
        get_hotels_with_rooms = (
            select(
                ...
                booked_hotels.c.number_of_rooms_left,
            )
